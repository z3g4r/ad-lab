...
.SYNOPSIS
    ...
    using the Vagrant SSH private key to reach linux-srv01.
...
.DESCRIPTION
    This version is tailored for the FrostyLabs ad-lab layout:
      ...
      - Authenticate to Linux with an SSH private key copied from the Vagrant host
      - Use Windows OpenSSH client (ssh.exe / scp.exe)
...
.NOTES
    Before running this script on win-dc01:
      1. Copy the Vagrant SSH private key for linux-srv01 onto win-dc01
    ...
    LAB ONLY:
      The SSH options intentionally relax host-key persistence for convenience in a disposable lab.
#>
...
function Show-KeytabEntries {
    ...
    Write-Host "Key information from retrieved keytab:" -ForegroundColor Yellow
    Invoke-External -FilePath 'ktpass.exe' -Arguments @('/in', $KeytabPath) -Description "Inspect retrieved keytab"
    Write-Host ""
}
