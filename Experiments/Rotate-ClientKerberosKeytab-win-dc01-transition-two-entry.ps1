...
.DESCRIPTION
    ...
    Actions:
      ...
      Rotate
        ...
        - Merge OLD + FUTURE keytabs ON WIN-DC01 in a dedicated helper function
        - Inspect the merged transition keytab
        - Deploy the transition keytab to the client host
...
function Merge-KeytabsOnDomainController {
    param(
        [Parameter(Mandatory = $true)][string]$OldKeytabPath,
        [Parameter(Mandatory = $true)][string]$NewKeytabPath,
        [Parameter(Mandatory = $true)][string]$OutKeytabPath
    )

    if (-not (Test-Path -LiteralPath $OldKeytabPath)) {
        throw "Old keytab not found: $OldKeytabPath"
    }
    if (-not (Test-Path -LiteralPath $NewKeytabPath)) {
        throw "New keytab not found: $NewKeytabPath"
    }
    ...
    $ktutilScriptPath = Join-Path $script:AccountDir 'merge-client-transition.ktutil'
    @"
read_kt $OldKeytabPath
read_kt $NewKeytabPath
write_kt $OutKeytabPath
quit
"@ | Set-Content -LiteralPath $ktutilScriptPath -Encoding ascii -Force

    Write-Host ""
    Write-Host "Merging client transition keytab on win-dc01" -ForegroundColor Yellow
    Write-Host "---------------------------------------------"
    Write-Host "Old keytab: $OldKeytabPath"
    Write-Host "New keytab: $NewKeytabPath"
    Write-Host "Out keytab: $OutKeytabPath"
    Write-Host ""

    Get-Content -LiteralPath $ktutilScriptPath | & ktutil.exe
    if ($LASTEXITCODE -ne 0) {
        throw "ktutil.exe failed while merging client keytabs."
    }

    if (-not (Test-Path -LiteralPath $OutKeytabPath)) {
        throw "Merged client transition keytab was not created: $OutKeytabPath"
    }
}
...
"@
    Invoke-SSH -RemoteCommand "bash -lc \"$($remoteScript.Replace('"','\"'))\""
}
...
Assert-Command -Name 'ktutil.exe'
...
switch ($Action) {
    ...
    'Rotate' {
        ...
        Merge-KeytabsOnDomainController `
            -OldKeytabPath $oldKeytab `
            -NewKeytabPath $futureKeytab `
            -OutKeytabPath $transitionKeytab
